...
func getContainers(namespace, depName, registry, image string) []corev1.Container {
...
func (s *SidecarInjector) Handle(ctx context.Context, r admission.Request) admission.Response {
	...
	if checkInject(pod) {
		...
		pod.Spec.Containers = append(pod.Spec.Containers, getContainers(s.Namespace, pod.Annotations["cmsnr.com/deploymentName"], s.Registry, s.Image)...)
